...
<template>
  <div class="page">
    ...
    <form class="card form" @submit.prevent="submit">
      <input v-model="form.name" class="input" placeholder="Nama kategori (cth: Makanan)" />
      <input v-model="form.color" type="color" class="color" />
      <button class="btn" :disabled="saving">{{ saving ? 'Menyimpan...' : '+ Tambah' }}</button>
    </form>
...
.error { color: #e8442a; font-size: 13px; margin: 8px 0; }
.card { border: 2px solid #111; border-radius: 12px; background: #fff; box-shadow: 3px 3px 0 #111; }
.form { display: flex; gap: 8px; padding: 12px; margin-bottom: 1rem; align-items: center; }
.input { flex: 1; padding: 8px 12px; border: 2px solid #111; border-radius: 8px; font-size: 14px; }
.color { width: 44px; height: 38px; border: 2px solid #111; border-radius: 8px; cursor: pointer; }
.btn { padding: 8px 16px; border: 2px solid #111; border-radius: 8px; background: #2ABFA3; color: #fff; font-weight: 600; cursor: pointer; }
...
.dot { width: 16px; height: 16px; border-radius: 50%; border: 2px solid #111; }
